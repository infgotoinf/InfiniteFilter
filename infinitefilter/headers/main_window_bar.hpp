...
void drawGui2(const char* filename, int* img_width, int* img_height
...
void drawGui(std::string* filename, int* img_width, int* img_height
    ...
    if (ImGuiFileDialog::Instance()->Display("ChooseFileDlgKey", 0, minSize, maxSize))
    {
        if (ImGuiFileDialog::Instance()->IsOk()) // action if OK
        {
            ...
            *filename = ImGuiFileDialog::Instance()->GetFilePathName();
            ...
            bool ret = loadTextureFromFile((*filename).c_str(), renderer, texture
...
static void showFilterMenu(SDL_Renderer* renderer, SDL_Texture** texture)
{
    ...
    //             filter_image(file_data, img_width, img_height, channels, filename.c_str()
    //                        , renderer, texture);
    //         }
    //     }
    // }
